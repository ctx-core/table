...
import { row_id_b, row_id_Ctx } from './row_id_b'
import { table_b, table_Ctx } from './table_b'
import { rows_b } from './rows_b'
import { row_b, row_Ctx } from './row_b'
import type { Row } from './Row'
const key = 'set_row'
export interface set_row_Ctx<Val extends unknown = unknown>
	extends row_Ctx<Val>, row_id_Ctx, table_Ctx<Val> {
	set_row?:()=>void
}
export function set_row_b<Val extends unknown = unknown>(
	ctx:set_row_Ctx<Val>
) {
	return _b<set_row_Ctx<Val>, typeof key>(key, ()=>{
		if (has_dom) {
